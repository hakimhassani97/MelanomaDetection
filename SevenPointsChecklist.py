...
if __name__ == '__main__' :
    ...
    for file in files:
        ...
        # get color
        sensibility = SevenPointsChecklist.sensibility(img, contour)
        print('['+str(i)+'/'+str(len(files))+']',sensibility)
        t.append(sensibility)
        i += 1
        # draw text
        x, y = 0, np.shape(img)[0]-3
        cv2.putText(img,'evolution : '+str(sensibility), (x,y), cv2.FONT_HERSHEY_SIMPLEX, fontScale = 0.5, color = (0, 0, 0), lineType = 1)
        cv2.imshow('img',img)
